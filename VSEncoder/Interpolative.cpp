...
void
BitWriter::encode_interpolative(const vector<unsigned int> &pos, int j, int k)
{
    while (j + 1 < k) {
...
unsigned int
BitReader::decode(unsigned int outof, bool force)
{
    (void)force;
    size_t bits = highest_order_bit(outof - 1);
    const size_t spare = (1 << bits) - outof;
...
BitReader::decode_interpolative_next()
{
	...
	if (!di_current.is_next()) {
	    unsigned int pos_ret = di_current.pos_k;
